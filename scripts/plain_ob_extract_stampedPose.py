...
class ExtractObjectivepose:
    def __init__(self):
        ...
        self.ob_pub = rospy.Publisher("/objective_pose", PoseStamped, queue_size = 1)
        ...
        rospy.Subscriber("/laser_feature_capture_reflect_board", Bool, self.open_callback)
        ...
        if len(xs) > 20:
            ...
            for board in backboards:
                ...
                if segs[1].flen < self.board_mid_len_max \
                    ...
                    pose.header.stamp = data.header.stamp   #按time(0)进行帧转换，但发布之前还是改为当前时间
                    ob.poses.append(pose.pose)          #允许存在多个合格板子，都放到ob.poses，供base_link节点选择最近的
                    rospy.loginfo("~~~~~~~~~~~~~~~~~~~~~~~~")
                    self.ob_pub.publish(pose)
                    return
        pose = PoseStamped()
